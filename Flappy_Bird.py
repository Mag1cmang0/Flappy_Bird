...
def main(genomes, config):
    ...
    while run:
        ...
        pipe_index = 0
        if len(birds) > 0:
            if len(pipes) > 0 and birds[0].x > pipes[0].x + pipes[0].PIPE_BOTTOM.get_width():
                pipe_index = 1
        else:
            run = False
            break
        ...
        for x, bird in enumerate(birds):
            ...
            nn_output = nets[x].activate((bird.y , abs(bird.y - pipes[pipe_index].height), abs(bird.y - pipes[pipe_index].bottom)))
        ...
        for pipe in pipes:
            ...
            for x, bird in enumerate(birds):
                # if the bird hits the pipe, pass for now
